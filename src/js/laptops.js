...
function generarLaptops() {
    async function cargarProdctos() {
         try {
             const respuesta = await fetch('src\\js\\productos.json');
             const resultado = await respuesta.json();    
             const laptops = resultado.productos.computadoras;
             const laptopsContainer = document.querySelector('.laptops-section__body-wrapper');

             laptops.forEach(laptop => {
                const laptopDiv = document.createElement('DIV');
                laptopDiv.innerHTML = `
                <div class="laptops-section__item item" data-id="${laptop.id}">
                    <div class="laptops-section__item-img-wrapper">
                        <img src="${laptop.imagen}" alt="product-img">
                    </div>
                    <div class="laptops-section__item-title-wrapper">
                        <h2 class="laptops-section__item-title">${laptop.nombre}</h2>
                    </div>
                    <div class="laptops-section__item-price-wrapper">
                        <p class="laptops-section__item-price">$${laptop.precio}</p>
                    </div>
                    <div class="laptops-section__item-btn-wrapper">
                        <button class="laptops-section__item-btn btn btn-primary btn-item-carro"><a href="#">Agregar al
                                Carro</a></button>
