...
function generarConsolas() {
    async function cargarProdctos() {
         try {
             const respuesta = await fetch('src\\js\\productos.json');
             const resultado = await respuesta.json();    
             const consolas = resultado.productos.consolas;
             const consolasContainer = document.querySelector('.consolas-section__body-wrapper');

             consolas.forEach(consola => {
                const consolaDiv = document.createElement('DIV');
                consolaDiv.innerHTML = `
                <div class="consolas-section__item item" data-id="${consola.id}">
                    <div class="consolas-section__item-img-wrapper">
                        <img src="${consola.imagen}" alt="product-img">
                    </div>
                    <div class="consolas-section__item-title-wrapper">
                        <h2 class="consolas-section__item-title">${consola.nombre}</h2>
                    </div>
                    <div class="consolas-section__item-price-wrapper">
                        <p class="consolas-section__item-price">$${consola.precio}</p>
                    </div>
                    <div class="consolas-section__item-btn-wrapper">
                        <button class="consolas-section__item-btn btn btn-primary btn-item-carro"><a href="#">Agregar al
                                Carro</a></button>
