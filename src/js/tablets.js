...
function generarTablets() {
    async function cargarProdctos() {
         try {
             const respuesta = await fetch('src\\js\\productos.json');
             const resultado = await respuesta.json();    
             const tablets = resultado.productos.tablets;
             const tabletsContainer = document.querySelector('.tablets-section__body-wrapper');

             tablets.forEach(tablet => {
                const tabletDiv = document.createElement('DIV');
                tabletDiv.innerHTML = `
                <div class="tablets-section__item item" data-id="${tablet.id}">
                    <div class="tablets-section__item-img-wrapper">
                        <img src="${tablet.imagen}" alt="product-img">
                    </div>
                    <div class="tablets-section__item-title-wrapper">
                        <h2 class="tablets-section__item-title">${tablet.nombre}</h2>
                    </div>
                    <div class="tablets-section__item-price-wrapper">
                        <p class="tablets-section__item-price">$${tablet.precio}</p>
                    </div>
                    <div class="tablets-section__item-btn-wrapper">
                        <button class="tablets-section__item-btn btn btn-primary btn-item-carro"><a href="#">Agregar al
                                Carro</a></button>
            ...
            tabletsContainer.appendChild(tabletDiv);
         });
