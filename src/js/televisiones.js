...
function generarTelevisiones() {
    async function cargarProdctos() {
         try {
             const respuesta = await fetch('src\\js\\productos.json');
             const resultado = await respuesta.json();    
             const televisiones = resultado.productos.televisiones;
             const televisionesContainer = document.querySelector('.televisiones-section__body-wrapper');

             televisiones.forEach(television => {
                const televisionDiv = document.createElement('DIV');
                televisionDiv.innerHTML = `
                <div class="televisiones-section__item item" data-id="${television.id}">
                    <div class="televisiones-section__item-img-wrapper">
                        <img src="${television.imagen}" alt="product-img">
                    </div>
                    <div class="televisiones-section__item-title-wrapper">
                        <h2 class="televisiones-section__item-title">${television.nombre}</h2>
                    </div>
                    <div class="televisiones-section__item-price-wrapper">
                        <p class="televisiones-section__item-price">$${television.precio}</p>
                    </div>
                    <div class="televisiones-section__item-btn-wrapper">
                        <button class="televisiones-section__item-btn btn btn-primary btn-item-carro"><a href="#">Agregar al
                                Carro</a></button>
            ...
            televisionesContainer.appendChild(televisionDiv);
         });
